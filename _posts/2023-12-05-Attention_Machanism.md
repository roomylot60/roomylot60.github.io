...
date: 2023-12-05 20:12:00-0400
description: Attention to resolve vanishing gradient problem in RNN
tags: NLP
categories: concept
...
- Attention Score : 현재 decoder의 `t 시점`에서의 단어를 예측하기 위해 encoder의 모든 hidden state의 값(`h_{i}`)이 decoder의 현재 hidden state(`s_{t}`)와 얼마나 유사한지를 판단하는 값으로, s__{t}^{T}와 h_{i}의 dot product(내적) 결과로 모두 Scalar 값; `score(s_{t}, h_{i}) =  s__{t}^{T}h_{i}` ![Attention scores](../Attatched/Pasted%20image%2020240103143640.png)
(e^{t}는 모든 score의 모음값)
